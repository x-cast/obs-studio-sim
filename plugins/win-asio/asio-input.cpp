...
static bool asio_device_changed(void *vptr, obs_properties_t *props,
				obs_property_t *list, obs_data_t *settings);
static bool asio_layout_changed(obs_properties_t *props, obs_property_t *list,
				obs_data_t *settings);
static bool fill_out_channels_modified(obs_properties_t *props,
				       obs_property_t *list,
				       obs_data_t *settings);
...
public:
	...
	int write_index() { return _write_index; }
...
public:
	...
	private:
		...
		bool set_data(AudioBufferInfo *info, obs_source_audio &out,
			      const std::vector<short> &route, int *sample_rate)
		{
			...
			*sample_rate = out.samples_per_sec;
			...
			size_t buffer_size =
				(out.frames *
				 sizeof(bytedepth_format(out.format)));
	...
	public:
		AudioListener(obs_source_t *source, AudioCB *cb)
			: source(source), callback(cb)
		{
			active = true;
		}
		...
		void setCurrentCallback(AudioCB *cb) { current_callback = cb; }

		void setCallback(AudioCB *cb) { callback = cb; }

		void setReadIndex(int idx) { read_index = idx; }

		void setRoute(std::vector<short> route) { _route = route; }

		AudioCB *getCallback() { return callback; }
		...
		int useTimeSlice()
		{
			...
			while (read_index != write_index) {
				obs_source_audio out;
				bool unmuted =
					set_data(&callback->buffers[read_index],
						 out, route, &sample_rate);
			...
			wait_time = ((1000 / 2) * AUDIO_OUTPUT_FRAMES) /
				    max_sample_rate;
			return wait_time;
		}
	};

	AudioIODevice *getDevice() { return _device; }
				   ...
				   int numOutputChannels, int numSamples)
	{
		...
		for (int i = 0; i < numInputChannels; i++)
			buffers[_write_index].buffer.copyFrom(
				i, 0, inputChannelData[i], numSamples);
		buffers[_write_index].out.timestamp = ts;
		buffers[_write_index].out.frames = numSamples;
		buffers[_write_index].out.samples_per_sec =
	...
	{
		blog(LOG_INFO, "Starting (%s)",
		     device->getName().toStdString().c_str());
		...
		for (; i < buffers.size(); i++) {
			buffers[i].buffer =
				AudioBuffer<float>(ch_count, buf_size);
	...
	{
		blog(LOG_INFO, "Stopped (%s)",
		     _device->getName().toStdString().c_str());
	}
...
static bool show_panel(obs_properties_t *props, obs_property_t *property,
		       void *data);
...
public:
	...
	// version of plugin
	static bool credits(obs_properties_t *props, obs_property_t *property,
			    void *data)
	{
	...
	static obs_properties_t *Properties(void *vptr)
	{
		...
		obs_property_set_visible(panel,
					 device && device->hasControlPanel());
		button = obs_properties_add_button(props, "credits", "CREDITS",
						   credits);
		return props;
	}
	...
	void update(obs_data_t *settings)
	{
		...
		speaker_layout layout = (speaker_layout)obs_data_get_int(
			settings, "speaker_layout");
		...
		if (!_device->isOpen()) {
			err = _device->open(
				in, out, _device->getCurrentSampleRate(),
		...
		if (callback) {
			...
			for (int i = 0; i < recorded_channels; i++) {
				...
				r.push_back(obs_data_get_int(
					settings, route_str.c_str()));
			}
				       ...
				       obs_data_t *settings)
{
	...
	for (; i < input_channels; i++)
		obs_property_list_add_int(list,
					  in_names[i].toStdString().c_str(), i);
				...
				obs_property_t *list, obs_data_t *settings)
{
	...
	speaker_layout layout =
		(speaker_layout)obs_data_get_int(settings, "speaker_layout");
	...
	if (!itemFound) {
		...
		for (i = 0; i < max_channels; i++) {
			...
			obs_property_set_visible(route[i],
						 i < recorded_channels);
		}
	}
...
static void fill_out_devices(obs_property_t *prop)
{
	...
	for (int j = 0; j < deviceNames.size(); j++) {
		...
		if (!found) {
			char *name =
				bstrdup(deviceNames[j].toStdString().c_str());
