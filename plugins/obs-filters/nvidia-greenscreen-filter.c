...
static void init_images_greenscreen(struct nv_greenscreen_data *filter)
{
	...
	if (vfxErr != NVCV_SUCCESS) {
		char *errString = NvCV_GetErrorStringFromCode(vfxErr);
...
static void nv_greenscreen_filter_render(void *data, gs_effect_t *effect)
{
	...
	if (!filter->initial_render) {
		...
		if (vfxErr != NVCV_SUCCESS) {
			char *errString = NvCV_GetErrorStringFromCode(vfxErr);
			error("Error creating src img; error %i: ùs", vfxErr,
			      errString);
			filter->processing_stop = true;
			return;
		}
	...
	if (filter->initial_render && filter->images_allocated) {
		if (process_texture_greenscreen(filter)) {
			if (!srgb_draw)
...
bool load_nvvfx(void)
{
	if (!load_nv_vfx_libs()) {
		blog(LOG_INFO,
		     ...
		     "Download here: https://www.nvidia.com/en-us/geforce/broadcasting/broadcast-sdk/resources/ \n");
		return false;
	}
