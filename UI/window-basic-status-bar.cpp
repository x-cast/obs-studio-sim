...
{
	obs_data_t *settings = streamOutput != nullptr ? obs_output_get_settings(streamOutput) : nullptr;
	bool dyn = false;
	if (settings) {
...
void OBSBasicStatusBar::UpdateDynamicBitrateStatus()
{
	...
	case BITRATE_SWITCHING_UP:
		...
		dynStatus->setToolTip(QTStr("Basic.StatusBar.DynamicBitrate.Up"));
		break;
	case BITRATE_SWITCHING_DOWN:
		dynStatus->setPixmap(dynPixmapDown);
		dynStatus->setToolTip(QTStr("Basic.StatusBar.DynamicBitrate.Down"));
		break;
	case BITRATE_SWITCHING_STATIONARY:
		dynStatus->setPixmap(dynPixmap);
		dynStatus->setToolTip(QTStr("Basic.StatusBar.DynamicBitrate.Stationary"));
		break;
	default:
		dynStatus->setPixmap(transparentPixmap);
	}
