...
void OBSBasic::CreateFirstRunSources()
{
	...
	//bool hasInputAudio = HasAudioDevices(App()->InputAudioSource()) || HasAudioDevices("asio_input_capture");
...
bool OBSBasic::InitBasicConfigDefaults()
{
	...
	config_set_default_int   (basicConfig, "SimpleOutput",
			"DynamicBitrateThreshold", 3);
	config_set_default_int   (basicConfig, "SimpleOutput",
			"DynamicBitrateRecoveryTime", 30);
	config_set_default_int   (basicConfig, "SimpleOutput",
			"DynamicBitrateDecreaseTime", 100);
	config_set_default_int   (basicConfig, "SimpleOutput",
			"DynamicBitrateFloor", 50);
	...
	config_set_default_int   (basicConfig, "AdvOut", "DynamicBitrateDecreaseTime",
			100);
	config_set_default_int   (basicConfig, "AdvOut", "DynamicBitrateFloor", 50);
	...
	config_set_default_bool(basicConfig, "AdvOut", "StreamFFUseRescale",
			false);
	config_set_default_bool(basicConfig, "AdvOut", "StreamFFIgnoreCompat",
			true);
...
void OBSBasic::UnhideAllMasterAudioControls()
{
	for (int i = 0; i < MAX_AUDIO_MIXES; i++) {
		obs_data_t *private_settings =
				obs_source_get_private_settings(master_volumes[i]->GetSource());
...
void OBSBasic::MixerRenameSource()
{
	...
	for (;;) {
		...
		if (obs_source_get_output_flags(source) & OBS_SOURCE_TRACK) {
			...
			std::string trackNum = "Track" + std::to_string(track_index + 1) + "Name";
			vol->setObjectName(QString::fromStdString(name));
			OBSBasic *main = reinterpret_cast<OBSBasic*>(App()->GetMainWindow());
			config_set_string(main->Config(), "AdvOut", trackNum.c_str(), name.c_str());
			main->ResetOutputs();
...
void OBSBasic::VolControlContextMenu() {
	...
	showTracksButtonsAction.setChecked(config_get_bool(GetGlobalConfig(),
			"BasicWindow", "ShowTracksButtons"));
...
void OBSBasic::ActivateAudioSource(OBSSource source)
{
	...
	bool vertical = config_get_bool(GetGlobalConfig(), "BasicWindow",
			"VerticalVolControl");
	bool ShowTracksButtons = config_get_bool(GetGlobalConfig(), "BasicWindow",
			"ShowTracksButtons");
	VolControl *vol = new VolControl(source, NULL, true, vertical, ShowTracksButtons,
			SOURCE_IS_NOT_TRACK);
...
void OBSBasic::InitAudioMasterMixer()
{
	...
	for (int i = 0; i < MAX_AUDIO_MIXES; i++) {
		vol[i] = new VolControl(tracks[i], &muted[i], true, vertical, false, i);
		...
		if (nameAdv) {
			...
			name = nameStr.compare("") == 0 ? trackNum.c_str() : nameAdv;
		} else {
			name = trackNum.c_str();
		}
		if (!tracks[i]) {
...
QMenu *OBSBasic::CreateAddSourcePopupMenu()
{
	...
	while (obs_enum_input_types(idx++, &type)) {
		...
		if ((caps & OBS_SOURCE_CAP_DISABLED) != 0 || caps & OBS_SOURCE_TRACK)
			continue;
