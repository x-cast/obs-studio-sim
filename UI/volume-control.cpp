...
void VolControl::SetRec(bool checked)
{
	...
	int recbitmask;
	if (isStandard)
		recbitmask = config_get_int(main->Config(), "AdvOut", "RecTracks");
	else
		recbitmask = config_get_int(main->Config(), "AdvOut", "FFAudioMixes");
	...
	if (isStandard)
		...
		config_set_int(main->Config(), "AdvOut", "FFAudioMixes", newbitmask);
...
VolControl::VolControl(OBSSource source_, bool *mutePtr, bool showConfig,
		bool vertical, bool showTracks, int trackIndex)
		...
		vertical      (vertical)
{
	...
	if (vertical) {
		...
		if (trackIndex < 0)
			send->setProperty("themeID", "MacOnly");
		if (trackIndex >= 0) {
	 ...
	 * This allows switching between OBS_MONITORING_TYPE_MONITOR_ONLY and
	...
	if (trackIndex >= 0) {
		...
		stream->setToolTip(QTStr("VolControl.Stream.Tooltip").arg(track_index + 1));
		...
		rec->setToolTip(QTStr("VolControl.Rec.Tooltip").arg(track_index + 1));
